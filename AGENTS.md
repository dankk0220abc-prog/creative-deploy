...
Phase 1B is complete. The current implementation remains limited to the foundation health
slice:
...
The next phase has not started. The foundation health page is not the final PaintPilot UI.
Before any business page work begins, discuss UI/UX with the project owner. Codex must not
unilaterally apply a generic administration-dashboard template. The visual direction,
interaction flow, and information hierarchy require explicit user confirmation.
...
- Alembic, database tables, Redis, workers, CI, or production deployment.

## Directory Responsibilities

- `apps/api`: the independent uv-managed FastAPI package, health schemas, configuration,
  async database engine, readiness service, and pytest tests.
...
- Do not add business entities, tables, migrations, AI SDKs, Redis, app Dockerfiles, or
  production infrastructure while the approved task is limited to health checks.
